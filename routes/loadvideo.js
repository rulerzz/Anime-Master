...
// Fetch Gogoanime latest header links
  request('https://www.gogoanime1.com',
  (error, response, html) => {    
...
router.get('/', function(req, res, next) {
  ...
  request(seriesUrl,(error, response, html) => {    
    ...
    if(!error && response.statusCode == 200){
        ...
        detailsEpisode.downloadLink = $('.vmn-buttons').children().last().attr('href');
        res.render('playepisode', { title: 'Anime Master' ,navList : navList,detailsEpisode : detailsEpisode,seriesUrl : seriesUrl,episodeList : episodeList,url : url});
    }
    else{
        console.log(error);
    }
