...
router.get('/', function(req, res, next) {
    var latestk = [];
  request('https://mangakakalot.com/manga_list?type=newest&category=all&state=all&page=1',(error, response, html) => {    
      if(!error && response.statusCode == 200){
          const $ = cheerio.load(html);
             $('.truyen-list .list-truyen-item-wrap').each((j , eld) => {
              ...
              res.render('newmanga', { title: 'Anime Master' ,latest : latestk, total : total,links : linksk});
      }
      else{
          console.log(error);
      }
